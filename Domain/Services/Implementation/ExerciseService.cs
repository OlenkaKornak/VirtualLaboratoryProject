...
namespace Domain.Services.Implementation
{
    public class ExerciseService : IExerciseService
    {
        ...
        public async Task<double> SaveExecution(string userId, SaveExecutionViewModel viewModel)
        {
        ...
        private double GetGrade(List<SaveAnswerViewModel> answers)
        {
            ...
            foreach(var answ in questions[0].Answers)
            {
                if (answ.IsCorrect)
                {
                    correctAnswers.Add(answ.Id);
                }
                else
                {
                    incorrectAnswers.Add(answ.Id);
                }
            }
            ...
            {
                foreach(var userCorrect in answers[0].Correct)
                {
                    if (correct == userCorrect)
                        ++grade;
                }
            }
            foreach (var incorrect in incorrectAnswers)
            {
                foreach (var userIncorrect in answers[0].Incorrect)
                {
                    if (incorrect == userIncorrect)
                        ++grade;
                }
            }

            return Math.Round(grade * 1.0 / (correctAnswers.Count() + incorrectAnswers.Count()) * 100, 1);
        }
    }
}
